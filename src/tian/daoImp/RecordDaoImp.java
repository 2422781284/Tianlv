...
public class RecordDaoImp extends BaseDaoImpl<Record> implements RecordDao {
    ...
    public Object findRec(Long id) {
        return getSession().createQuery("select record.date,record.formNumber,record.tele,record.addr from Record as record where record.id=?").setLong(0,id).uniqueResult();
    }
